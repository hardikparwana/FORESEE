...
def sigma_point_expand_JIT(robot_state, sigma_points, weights, cur_t, noise):
    ...
    return new_points, new_weights
traced_sigma_point_expand_JIT = torch.jit.script( sigma_point_expand_JIT, ( torch.ones(3,1), torch.ones(2,5), torch.ones(1,5), torch.tensor([1]), torch.tensor([0]) ) )
...
def sigma_point_expand_ideal_JIT(robot_state, sigma_points, weights, cur_t, noise):
    ...
    for i in range(1,N):
        ...
        new_weights = torch.cat( (new_weights, (temp_weights * weights[0,i]).reshape(1,-1) ) , dim=1 )
    return new_points, new_weights
traced_sigma_point_expand_ideal_JIT = torch.jit.script( sigma_point_expand_ideal_JIT, ( torch.ones(3,1), torch.ones(2,5), torch.ones(1,5), torch.tensor([1]), torch.tensor([0]) ) ) #sigma_point_expand_ideal_JIT
