...
def policy(param_w, param_mu, param_Sigma, X):
    ...
    i = 0
    Sigma = np.array([  
            [ param_Sigma[i,0], 0.0, 0.0, 0.0 ],
            [ param_Sigma[i,4], param_Sigma[i,1], 0.0, 0.0 ],
            [ param_Sigma[i,5], param_Sigma[i,6], param_Sigma[i,2], 0.0 ],
            [ param_Sigma[i,7], param_Sigma[i,8], param_Sigma[i,9], param_Sigma[i,3] ]
        ])
...
policy_grad_jit = jit(grad(policy, 0))
...
t0 = time.time()
test_u = policy_grad_jit( test_param_w, test_param_mu, test_param_Sigma, test_X )
print(f"time:{time.time()-t0}")
