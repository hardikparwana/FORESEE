...
def dynamics_xdot_noisy(state, action = np.array([0])):
    ...
    # skewness = xdot / 3
    # kurt = np.square(error_square)
    
    
    # xdot = xdot + xdot/2 
    # return xdot, cov, skewness, kurt
...
def compare_predictions(ax, horizons = 3, num_particles = 5000, expanded_only = False):
    ...
    foresee_genut_mu, foresee_genut_cov = get_mean_cov( sigma_points_GenUT, weights_GenUT, weights_GenUT )
    # _, foresee_complete_cov = get_mean_cov( full_sigma_points, full_weights1, full_weights2 )
...
# MC 500
ax_3.plot( [1, 2, 3],  [ mc_cov1[0,0], mc_cov7[0,0], mc_cov13[0,0] ], 'b', label='MC 500' )
# foresee_compressed
ax_3.plot( [1, 2, 3], [ foresee_cov1[0,0], foresee_cov7[0,0], foresee_cov13[0,0] ], 'r', label='UT Expansion Only' )
# foresee_expanded
ax_3.plot( [1, 2, 3], [ foresee_cov2[0,0], foresee_cov8[0,0], foresee_cov23[0,0] ], 'g', label='UT Expansion - Compression' )
# PILCO
ax_3.plot( [1, 2, 3], [ pilco_cov1[0,0], pilco_cov7[0,0], pilco_cov13[0,0] ], 'r--', label='Successive Gaussian' )

# foresee_compressed
ax_3.plot( [1, 2, 3], [ foresee_genut_cov1[1,1], foresee_genut_cov7[1,1], foresee_genut_cov13[1,1] ], 'r--', label='GenUT Expansion Only' )
# foresee_expanded
ax_3.plot( [1, 2, 3], [ foresee_genut_cov2[1,1], foresee_genut_cov8[1,1], foresee_genut_cov23[1,1] ], 'g--', label='GenUT Expansion - Compression' )


ax_3.legend()
ax_3.set_title('Predicted Covariance')
ax_3.set_xlabel('Prediction Horizon')
# ax_1.set_ylabel('Predicted Mean')
new_list = range(math.floor(1), math.ceil(3)+1)
plt.xticks(new_list)
# ax_1.xaxis.set_major_locator(mticker.MultipleLocator(1))
