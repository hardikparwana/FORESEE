...
def dynamics_xdot_noisy(state, action = np.array([0])):
    ...
    # gamma distribution
    k = 2.0
    theta = 0.01*np.square(xdot)+0.1
    # k = 1
    # theta = 2
    mean = k*theta
    cov = k*np.diag(theta[:,0]**2)
    skewness = np.array([2/np.sqrt(k),2/np.sqrt(k)]).reshape(-1,1)
    kurt = np.array([6/k+3, 6/k+3]).reshape(-1,1)
    return mean, cov, skewness, kurt, k, theta
...
# actual moments
def get_mean_cov_skew_kurt( sigma_points, weights, weights_cov ):
    # mean
    weighted_points = sigma_points * weights[0]
    mu = np.sum( weighted_points, 1 ).reshape(-1,1)    
    centered_points = sigma_points - mu    
...
#     weighted_centered_points = centered_points * weights[0] 
#     cov = weighted_centered_points @ centered_points.T    
    cov = np.diag(np.sum(centered_points**2 * weights_cov[0], axis=1))
    # Skewness times cov_root^-3
    skewness = np.sum(centered_points**3 * weights[0], axis=1) #/ cov[0,0]**(3/2) # for scipy    
    # kurtosis times cov_root^-4
...
def pilco_propagate(mean, cov):
    mu, cov, skewness, kurt, _, _ = dynamics_xdot_noisy(mean)
...
def mc_propagate(points):
    ...
    for i in range(points.shape[1]):
        mu, cov, skewness, kurt, k, theta = dynamics_xdot_noisy(points[:,i].reshape(-1,1))
        # sample = np.array([  np.random.normal(mu[0,0], np.sqrt(cov[0,0])), np.random.normal(mu[1,0], np.sqrt(cov[1,1]))  ]).reshape(-1,1) 
        sample = np.array([  np.random.gamma(k, theta[0,0]), np.random.gamma(k, theta[1,0])  ]).reshape(-1,1) 
...
def sigma_point_expand(sigma_points, weights1, weights2, control):
    ...
    # dt_outer = 0  
    #TODO  
    mu, cov, skewness, kurt, _, _ = dynamics_xdot_noisy(sigma_points[:,0].reshape(-1,1), control.reshape(-1,1))
    ...
    for i in range(1,N):
        mu, cov, skewness, kurt, _, _ = dynamics_xdot_noisy(sigma_points[:,i].reshape(-1,1), control.reshape(-1,1))
        root_term = get_ut_cov_root_diagonal(cov)           
...
def sigma_point_expand_GenUT(sigma_points, weights, control):
    ...
    mu, cov, skewness, kurt, _, _ = dynamics_xdot_noisy(sigma_points[:,0].reshape(-1,1), control.reshape(-1,1))
    root_term = get_ut_cov_root_diagonal(cov) 
    temp_points, temp_weights = generate_sigma_points_gaussian_GenUT( mu, root_term, skewness, kurt, sigma_points[:,0].reshape(-1,1), dt )
    new_points = np.copy( temp_points )
    ...
    for i in range(1,N):
        mu, cov, skewness, kurt, _, _ = dynamics_xdot_noisy(sigma_points[:,i].reshape(-1,1), control.reshape(-1,1))
        root_term = get_ut_cov_root_diagonal(cov)           
        temp_points, temp_weights = generate_sigma_points_gaussian_GenUT( mu, root_term, skewness, kurt, sigma_points[:,i].reshape(-1,1), dt )
        new_points = np.append(new_points, temp_points, axis=1 )
...
def foresee_propagate_GenUT( sigma_points, weights, action = np.array([0]), expanded_only=False ):
    ...
    #Expansion Layer
    expanded_sigma_points, expanded_weights = sigma_point_expand_GenUT( sigma_points, weights, action )
...
def get_mc_predictions(horizons = 3, num_particles = 5000):
    ...
    for i in range(num_runs):
        ...
        mc_mu, mc_cov, mc_skewness, mc_kurt = get_mean_cov_skew_kurt(mc_particles, np.ones((1,mc_particles.shape[1]))/mc_particles.shape[1], np.ones((1,mc_particles.shape[1]))/mc_particles.shape[1])
...
def compare_predictions(ax, horizons = 3, num_particles = 5000, expanded_only = False):
    ...
    # Compute Cov
    mc_mu, mc_cov, mc_skewness, mc_kurt = get_mean_cov_skew_kurt(mc_particles, np.ones((1,mc_particles.shape[1]))/mc_particles.shape[1], np.ones((1,mc_particles.shape[1]))/mc_particles.shape[1])
    foresee_mu, foresee_cov, foresee_skewness, foresee_kurt = get_mean_cov_skew_kurt( sigma_points, weights1, weights2 )
    foresee_genut_mu, foresee_genut_cov, foresee_genut_skewness, foresee_genut_kurt = get_mean_cov_skew_kurt( sigma_points_GenUT, weights_GenUT, weights_GenUT)
...
if 1:
    ...
    mc2_500, mc2_500_std = get_mc_predictions(horizons = 5, num_particles = num_500);
    mc2_5000, mc2_5000_std = get_mc_predictions(horizons = 5, num_particles = num_5000);
    mc2_50000, mc2_50000_std = get_mc_predictions(horizons = 5, num_particles = num_50000);
    
    mc3_500, mc3_500_std = get_mc_predictions(horizons = 10, num_particles = num_500);
    mc3_5000, mc3_5000_std = get_mc_predictions(horizons = 10, num_particles = num_5000);
    mc3_50000, mc3_50000_std = get_mc_predictions(horizons = 10, num_particles = num_50000);
    
    mc4_500, mc4_500_std = get_mc_predictions(horizons = 15, num_particles = num_500);
    mc4_5000, mc4_5000_std = get_mc_predictions(horizons = 15, num_particles = num_5000);
    mc4_50000, mc4_50000_std = get_mc_predictions(horizons = 15, num_particles = num_50000);
    
    mc5_500, mc5_500_std = get_mc_predictions(horizons = 20, num_particles = num_500);
    mc5_5000, mc5_5000_std = get_mc_predictions(horizons = 20, num_particles = num_5000);
    mc5_50000, mc5_50000_std = get_mc_predictions(horizons = 20, num_particles = num_50000);
    
    mc6_500, mc6_500_std = get_mc_predictions(horizons = 25, num_particles = num_500);
    mc6_5000, mc6_5000_std = get_mc_predictions(horizons = 25, num_particles = num_5000);
    mc6_50000, mc6_50000_std = get_mc_predictions(horizons = 25, num_particles = num_50000);
    
    mc7_500, mc7_500_std = get_mc_predictions(horizons = 30, num_particles = num_500);
    mc7_5000, mc7_5000_std = get_mc_predictions(horizons = 30, num_particles = num_5000);
    mc7_50000, mc7_50000_std = get_mc_predictions(horizons = 30, num_particles = num_50000);
    
    mc8_500, mc8_500_std = get_mc_predictions(horizons = 35, num_particles = num_500);
    mc8_5000, mc8_5000_std = get_mc_predictions(horizons = 35, num_particles = num_5000);
    mc8_50000, mc8_50000_std = get_mc_predictions(horizons = 35, num_particles = num_50000);
    
    mc9_500, mc9_500_std = get_mc_predictions(horizons = 40, num_particles = num_500);
    mc9_5000, mc9_5000_std = get_mc_predictions(horizons = 40, num_particles = num_5000);
    mc9_50000, mc9_50000_std = get_mc_predictions(horizons = 40, num_particles = num_50000);
    
    mc10_500, mc10_500_std = get_mc_predictions(horizons = 45, num_particles = num_500);
    mc10_5000, mc10_5000_std = get_mc_predictions(horizons = 145, num_particles = num_5000);
    mc10_50000, mc10_50000_std = get_mc_predictions(horizons = 45, num_particles = num_50000);
    
    fig1, ax1 = plt.subplots(1, 1)
    pilco1, _, foresee1, foresee_genut1 = compare_predictions( ax1, horizons = 1, num_particles = 500, expanded_only = False )
    fig1.savefig(home+"v2fig1.png")

    fig2, ax2 = plt.subplots(1, 1)
    pilco2, _, foresee2, foresee_genut2 = compare_predictions( ax2, horizons = 5, num_particles = 500, expanded_only = False )
    fig2.savefig(home+"v2fig5.png")

    fig3, ax3 = plt.subplots(1, 1)
    pilco3, _, foresee3, foresee_genut3 = compare_predictions( ax3, horizons = 10, num_particles = 500, expanded_only = False )
    fig3.savefig(home+"v2fig10.png")

    fig4, ax4 = plt.subplots(1, 1)
    pilco4, _, foresee4, foresee_genut4 = compare_predictions( ax4, horizons = 15, num_particles = 500, expanded_only = False )
    fig4.savefig(home+"v2fig15.png")
    
    fig5, ax5 = plt.subplots(1, 1)
    pilco5, _, foresee5, foresee_genut5 = compare_predictions( ax5, horizons = 20, num_particles = 500, expanded_only = False )
    fig5.savefig(home+"v2fig20.png")

    fig6, ax6 = plt.subplots(1, 1)
    pilco6, _, foresee6, foresee_genut6 = compare_predictions( ax6, horizons = 25, num_particles = 500, expanded_only = False )
    fig6.savefig(home+"v2fig25.png")

    fig7, ax7 = plt.subplots(1, 1)
    pilco7, _, foresee7, foresee_genut7 = compare_predictions( ax7, horizons = 30, num_particles = 500, expanded_only = False )
    fig7.savefig(home+"v2fig30.png")

    fig8, ax8 = plt.subplots(1, 1)
    pilco8, _, foresee8, foresee_genut8 = compare_predictions( ax8, horizons = 35, num_particles = 500, expanded_only = False )
    fig8.savefig(home+"v2fig35.png")

    fig9, ax9 = plt.subplots(1, 1)
    pilco9, _, foresee9, foresee_genut9 = compare_predictions( ax9, horizons = 40, num_particles = 500, expanded_only = False )
    fig9.savefig(home+"v2fig40.png")

    fig10, ax10 = plt.subplots(1, 1)
    pilco10, _, foresee10, foresee_genut10 = compare_predictions( ax10, horizons = 45, num_particles = 500, expanded_only = False )
    fig10.savefig(home+"v2fig45.png")
...
horizon_steps = [1,5,10,15,20,25,30,35,40,45]
ax_1[0].plot(horizon_steps, [mc1_500[0], mc2_500[0], mc3_500[0], mc4_500[0], mc5_500[0], mc6_500[0], mc7_500[0], mc8_500[0], mc9_500[0], mc10_500[0] ], 'green', label='MC 500')#'green'  )
ax_1[0].plot(horizon_steps, [mc1_5000[0], mc2_5000[0], mc3_5000[0], mc4_5000[0], mc5_5000[0], mc6_5000[0], mc7_5000[0], mc8_5000[0], mc9_5000[0], mc10_5000[0] ], 'b', label='MC 5000')#'b'  )
ax_1[0].plot(horizon_steps, [mc1_50000[0], mc2_50000[0], mc3_50000[0], mc4_50000[0], mc5_50000[0], mc6_50000[0], mc7_50000[0], mc8_50000[0], mc9_50000[0], mc10_50000[0] ], 'fuchsia', label='MC 50000')#'fuchsia'  )
ax_1[0].plot(horizon_steps, [pilco1[0], pilco2[0], pilco3[0], pilco4[0], pilco5[0], pilco6[0], pilco7[0], pilco8[0], pilco9[0], pilco10[0] ], 'r' , label='Successive Gaussian' )
ax_1[0].plot(horizon_steps, [foresee1[0], foresee2[0], foresee3[0], foresee4[0], foresee5[0], foresee6[0], foresee7[0], foresee8[0], foresee9[0], foresee10[0] ], 'y', label='EC - UT'  )
ax_1[0].plot(horizon_steps, [foresee_genut1[0], foresee_genut2[0], foresee_genut3[0], foresee_genut4[0], foresee_genut5[0], foresee_genut6[0], foresee_genut7[0], foresee_genut8[0], foresee_genut9[0], foresee_genut10[0] ], 'salmon' , label='EC - GenUT' )

ax_1[1].plot(horizon_steps, [mc1_500[1], mc2_500[1], mc3_500[1], mc4_500[1], mc5_500[1], mc6_500[1], mc7_500[1], mc8_500[1], mc9_500[1], mc10_500[1] ], 'green', label='MC 500'  )
ax_1[1].plot(horizon_steps, [mc1_5000[1], mc2_5000[1], mc3_5000[1], mc4_5000[1], mc5_5000[1], mc6_5000[1], mc7_5000[1], mc8_5000[1], mc9_5000[1], mc10_5000[1] ], 'b', label='MC 5000'  )
ax_1[1].plot(horizon_steps, [mc1_50000[1], mc2_50000[1], mc3_50000[1], mc4_50000[1], mc5_50000[1], mc6_50000[1], mc7_50000[1], mc8_50000[1], mc9_50000[1], mc10_50000[1] ], 'fuchsia', label='MC 50000'  )
ax_1[1].plot(horizon_steps, [pilco1[1], pilco2[1], pilco3[1], pilco4[1], pilco5[1], pilco6[1], pilco7[1], pilco8[1], pilco9[1], pilco10[1] ], 'r', label='Successive Gaussian'  )
ax_1[1].plot(horizon_steps, [foresee1[1], foresee2[1], foresee3[1], foresee4[1], foresee5[1], foresee6[1], foresee7[1], foresee8[1], foresee9[1], foresee10[1] ], 'y', label='EC - UT'  )
ax_1[1].plot(horizon_steps, [foresee_genut1[1], foresee_genut2[1], foresee_genut3[1], foresee_genut4[1], foresee_genut5[1], foresee_genut6[1], foresee_genut7[1], foresee_genut8[1], foresee_genut9[1], foresee_genut10[1] ], 'salmon', label='EC - GenUT'  )

ax_1[0].fill_between( horizon_steps, [mc1_500[0]-1.96*mc1_500_std[0], mc2_500[0]-1.96*mc2_500_std[0], mc3_500[0]-1.96*mc3_500_std[0], mc4_500[0]-1.96*mc4_500_std[0], mc5_500[0]-1.96*mc5_500_std[0], mc6_500[0]-1.96*mc6_500_std[0], mc7_500[0]-1.96*mc7_500_std[0], mc8_500[0]-1.96*mc8_500_std[0], mc9_500[0]-1.96*mc9_500_std[0], mc10_500[0]-1.96*mc10_500_std[0] ], [mc1_500[0]+1.96*mc1_500_std[0], mc2_500[0]+1.96*mc2_500_std[0], mc3_500[0]+1.96*mc3_500_std[0], mc4_500[0]+1.96*mc4_500_std[0], mc5_500[0]+1.96*mc5_500_std[0], mc6_500[0]+1.96*mc6_500_std[0], mc7_500[0]+1.96*mc7_500_std[0], mc8_500[0]+1.96*mc8_500_std[0], mc9_500[0]+1.96*mc9_500_std[0], mc10_500[0]+1.96*mc10_500_std[0] ], color='lime', alpha=0.30 )
ax_1[0].fill_between( horizon_steps, [mc1_5000[0]-1.96*mc1_5000_std[0], mc2_5000[0]-1.96*mc2_5000_std[0], mc3_5000[0]-1.96*mc3_5000_std[0], mc4_5000[0]-1.96*mc4_5000_std[0], mc5_5000[0]-1.96*mc5_5000_std[0], mc6_5000[0]-1.96*mc6_5000_std[0], mc7_5000[0]-1.96*mc7_5000_std[0], mc8_5000[0]-1.96*mc8_5000_std[0], mc9_5000[0]-1.96*mc9_5000_std[0], mc10_5000[0]-1.96*mc10_5000_std[0] ], [mc1_5000[0]+1.96*mc1_5000_std[0], mc2_5000[0]+1.96*mc2_5000_std[0], mc3_5000[0]+1.96*mc3_5000_std[0], mc4_5000[0]+1.96*mc4_5000_std[0], mc5_5000[0]+1.96*mc5_5000_std[0], mc6_5000[0]+1.96*mc6_5000_std[0], mc7_5000[0]+1.96*mc7_5000_std[0], mc8_5000[0]+1.96*mc8_5000_std[0], mc9_5000[0]+1.96*mc9_5000_std[0], mc10_5000[0]+1.96*mc10_5000_std[0] ], color='cyan', alpha=0.30 )
ax_1[0].fill_between( horizon_steps, [mc1_50000[0]-1.96*mc1_50000_std[0], mc2_50000[0]-1.96*mc2_50000_std[0], mc3_50000[0]-1.96*mc3_50000_std[0], mc4_50000[0]-1.96*mc4_50000_std[0], mc5_50000[0]-1.96*mc5_50000_std[0], mc6_50000[0]-1.96*mc6_50000_std[0], mc7_50000[0]-1.96*mc7_50000_std[0], mc8_50000[0]-1.96*mc8_50000_std[0], mc9_50000[0]-1.96*mc9_50000_std[0], mc10_50000[0]-1.96*mc10_50000_std[0] ], [mc1_50000[0]+1.96*mc1_50000_std[0], mc2_50000[0]+1.96*mc2_50000_std[0], mc3_50000[0]+1.96*mc3_50000_std[0], mc4_50000[0]+1.96*mc4_50000_std[0], mc5_50000[0]+1.96*mc5_50000_std[0], mc6_50000[0]+1.96*mc6_50000_std[0], mc7_50000[0]+1.96*mc7_50000_std[0], mc8_50000[0]+1.96*mc8_50000_std[0], mc9_50000[0]+1.96*mc9_50000_std[0], mc10_50000[0]+1.96*mc10_50000_std[0] ], color='fuchsia', alpha=0.30 )

ax_1[1].fill_between( horizon_steps, [mc1_500[1]-1.96*mc1_500_std[1], mc2_500[1]-1.96*mc2_500_std[1], mc3_500[1]-1.96*mc3_500_std[1], mc4_500[1]-1.96*mc4_500_std[1], mc5_500[1]-1.96*mc5_500_std[1], mc6_500[1]-1.96*mc6_500_std[1], mc7_500[1]-1.96*mc7_500_std[1], mc8_500[1]-1.96*mc8_500_std[1], mc9_500[1]-1.96*mc9_500_std[1], mc10_500[1]-1.96*mc10_500_std[1] ], [mc1_500[1]+1.96*mc1_500_std[1], mc2_500[1]+1.96*mc2_500_std[1], mc3_500[1]+1.96*mc3_500_std[1], mc4_500[1]+1.96*mc4_500_std[1], mc5_500[1]+1.96*mc5_500_std[1], mc6_500[1]+1.96*mc6_500_std[1], mc7_500[1]+1.96*mc7_500_std[1], mc8_500[1]+1.96*mc8_500_std[1], mc9_500[1]+1.96*mc9_500_std[1], mc10_500[1]+1.96*mc10_500_std[1] ], color='lime', alpha=0.30 )
ax_1[1].fill_between( horizon_steps, [mc1_5000[1]-1.96*mc1_5000_std[1], mc2_5000[1]-1.96*mc2_5000_std[1], mc3_5000[1]-1.96*mc3_5000_std[1], mc4_5000[1]-1.96*mc4_5000_std[1], mc5_5000[1]-1.96*mc5_5000_std[1], mc6_5000[1]-1.96*mc6_5000_std[1], mc7_5000[1]-1.96*mc7_5000_std[1], mc8_5000[1]-1.96*mc8_5000_std[1], mc9_5000[1]-1.96*mc9_5000_std[1], mc10_5000[1]-1.96*mc10_5000_std[1] ], [mc1_5000[1]+1.96*mc1_5000_std[1], mc2_5000[1]+1.96*mc2_5000_std[1], mc3_5000[1]+1.96*mc3_5000_std[1], mc4_5000[1]+1.96*mc4_5000_std[1], mc5_5000[1]+1.96*mc5_5000_std[1], mc6_5000[1]+1.96*mc6_5000_std[1], mc7_5000[1]+1.96*mc7_5000_std[1], mc8_5000[1]+1.96*mc8_5000_std[1], mc9_5000[1]+1.96*mc9_5000_std[1], mc10_5000[1]+1.96*mc10_5000_std[1] ], color='cyan', alpha=0.30 )
ax_1[1].fill_between( horizon_steps, [mc1_50000[1]-1.96*mc1_50000_std[1], mc2_50000[1]-1.96*mc2_50000_std[1], mc3_50000[1]-1.96*mc3_50000_std[1], mc4_50000[1]-1.96*mc4_50000_std[1], mc5_50000[1]-1.96*mc5_50000_std[1], mc6_50000[1]-1.96*mc6_50000_std[1], mc7_50000[1]-1.96*mc7_50000_std[1], mc8_50000[1]-1.96*mc8_50000_std[1], mc9_50000[1]-1.96*mc9_50000_std[1], mc10_50000[1]-1.96*mc10_50000_std[1] ], [mc1_50000[1]+1.96*mc1_50000_std[1], mc2_50000[1]+1.96*mc2_50000_std[1], mc3_50000[1]+1.96*mc3_50000_std[1], mc4_50000[1]+1.96*mc4_50000_std[1], mc5_50000[1]+1.96*mc5_50000_std[1], mc6_50000[1]+1.96*mc6_50000_std[1], mc7_50000[1]+1.96*mc7_50000_std[1], mc8_50000[1]+1.96*mc8_50000_std[1], mc9_50000[1]+1.96*mc9_50000_std[1], mc10_50000[1]+1.96*mc10_50000_std[1] ], color='fuchsia', alpha=0.30 )
...
ax_2[0].plot(horizon_steps, [mc1_500[2], mc2_500[2], mc3_500[2], mc4_500[2], mc5_500[2], mc6_500[2], mc7_500[2], mc8_500[2], mc9_500[2], mc10_500[2] ], 'green'  )
ax_2[0].plot(horizon_steps, [mc1_5000[2], mc2_5000[2], mc3_5000[2], mc4_5000[2], mc5_5000[2], mc6_5000[2], mc7_5000[2], mc8_5000[2], mc9_5000[2], mc10_5000[2] ], 'b'  )
ax_2[0].plot(horizon_steps, [mc1_50000[2], mc2_50000[2], mc3_50000[2], mc4_50000[2], mc5_50000[2], mc6_50000[2], mc7_50000[2], mc8_50000[2], mc9_50000[2], mc10_50000[2] ], 'fuchsia'  )
ax_2[0].plot(horizon_steps, [pilco1[2], pilco2[2], pilco3[2], pilco4[2], pilco5[2], pilco6[2], pilco7[2], pilco8[2], pilco9[2], pilco10[2] ], 'r'  )
ax_2[0].plot(horizon_steps, [foresee1[2], foresee2[2], foresee3[2], foresee4[2], foresee5[2], foresee6[2], foresee7[2], foresee8[2], foresee9[2], foresee10[2] ], 'y'  )
ax_2[0].plot(horizon_steps, [foresee_genut1[2], foresee_genut2[2], foresee_genut3[2], foresee_genut4[2], foresee_genut5[2], foresee_genut6[2], foresee_genut7[2], foresee_genut8[2], foresee_genut9[2], foresee_genut10[2] ], 'salmon'  )

ax_2[1].plot(horizon_steps, [mc1_500[3], mc2_500[3], mc3_500[3], mc4_500[3], mc5_500[3], mc6_500[3], mc7_500[3], mc8_500[3], mc9_500[3], mc10_500[3] ], 'green'  )
ax_2[1].plot(horizon_steps, [mc1_5000[3], mc2_5000[3], mc3_5000[3], mc4_5000[3], mc5_5000[3], mc6_5000[3], mc7_5000[3], mc8_5000[3], mc9_5000[3], mc10_5000[3] ], 'b'  )
ax_2[1].plot(horizon_steps, [mc1_50000[3], mc2_50000[3], mc3_50000[3], mc4_50000[3], mc5_50000[3], mc6_50000[3], mc7_50000[3], mc8_50000[3], mc9_50000[3], mc10_50000[3] ], 'fuchsia'  )
ax_2[1].plot(horizon_steps, [pilco1[3], pilco2[3], pilco3[3], pilco4[3], pilco5[3], pilco6[3], pilco7[3], pilco8[3], pilco9[3], pilco10[3] ], 'r'  )
ax_2[1].plot(horizon_steps, [foresee1[3], foresee2[3], foresee3[3], foresee4[3], foresee5[3], foresee6[3], foresee7[3], foresee8[3], foresee9[3], foresee10[3] ], 'y'  )
ax_2[1].plot(horizon_steps, [foresee_genut1[3], foresee_genut2[3], foresee_genut3[3], foresee_genut4[3], foresee_genut5[3], foresee_genut6[3], foresee_genut7[3], foresee_genut8[3], foresee_genut9[3], foresee_genut10[3] ], 'salmon'  )

ax_2[0].fill_between( horizon_steps, [mc1_500[2]-1.96*mc1_500_std[2], mc2_500[2]-1.96*mc2_500_std[2], mc3_500[2]-1.96*mc3_500_std[2], mc4_500[2]-1.96*mc4_500_std[2], mc5_500[2]-1.96*mc5_500_std[2], mc6_500[2]-1.96*mc6_500_std[2], mc7_500[2]-1.96*mc7_500_std[2], mc8_500[2]-1.96*mc8_500_std[2], mc9_500[2]-1.96*mc9_500_std[2], mc10_500[2]-1.96*mc10_500_std[2] ], [mc1_500[2]+1.96*mc1_500_std[2], mc2_500[2]+1.96*mc2_500_std[2], mc3_500[2]+1.96*mc3_500_std[2], mc4_500[2]+1.96*mc4_500_std[2], mc5_500[2]+1.96*mc5_500_std[2], mc6_500[2]+1.96*mc6_500_std[2], mc7_500[2]+1.96*mc7_500_std[2], mc8_500[2]+1.96*mc8_500_std[2], mc9_500[2]+1.96*mc9_500_std[2], mc10_500[2]+1.96*mc10_500_std[2] ], color='lime', alpha=0.30 )
ax_2[0].fill_between( horizon_steps, [mc1_5000[2]-1.96*mc1_5000_std[2], mc2_5000[2]-1.96*mc2_5000_std[2], mc3_5000[2]-1.96*mc3_5000_std[2], mc4_5000[2]-1.96*mc4_5000_std[2], mc5_5000[2]-1.96*mc5_5000_std[2], mc6_5000[2]-1.96*mc6_5000_std[2], mc7_5000[2]-1.96*mc7_5000_std[2], mc8_5000[2]-1.96*mc8_5000_std[2], mc9_5000[2]-1.96*mc9_5000_std[2], mc10_5000[2]-1.96*mc10_5000_std[2] ], [mc1_5000[2]+1.96*mc1_5000_std[2], mc2_5000[2]+1.96*mc2_5000_std[2], mc3_5000[2]+1.96*mc3_5000_std[2], mc4_5000[2]+1.96*mc4_5000_std[2], mc5_5000[2]+1.96*mc5_5000_std[2], mc6_5000[2]+1.96*mc6_5000_std[2], mc7_5000[2]+1.96*mc7_5000_std[2], mc8_5000[2]+1.96*mc8_5000_std[2], mc9_5000[2]+1.96*mc9_5000_std[2], mc10_5000[2]+1.96*mc10_5000_std[2] ], color='cyan', alpha=0.30 )
ax_2[0].fill_between( horizon_steps, [mc1_50000[2]-1.96*mc1_50000_std[2], mc2_50000[2]-1.96*mc2_50000_std[2], mc3_50000[2]-1.96*mc3_50000_std[2], mc4_50000[2]-1.96*mc4_50000_std[2], mc5_50000[2]-1.96*mc5_50000_std[2], mc6_50000[2]-1.96*mc6_50000_std[2], mc7_50000[2]-1.96*mc7_50000_std[2], mc8_50000[2]-1.96*mc8_50000_std[2], mc9_50000[2]-1.96*mc9_50000_std[2], mc10_50000[2]-1.96*mc10_50000_std[2] ], [mc1_50000[2]+1.96*mc1_50000_std[2], mc2_50000[2]+1.96*mc2_50000_std[2], mc3_50000[2]+1.96*mc3_50000_std[2], mc4_50000[2]+1.96*mc4_50000_std[2], mc5_50000[2]+1.96*mc5_50000_std[2], mc6_50000[2]+1.96*mc6_50000_std[2], mc7_50000[2]+1.96*mc7_50000_std[2], mc8_50000[2]+1.96*mc8_50000_std[2], mc9_50000[2]+1.96*mc9_50000_std[2], mc10_50000[2]+1.96*mc10_50000_std[2] ], color='fuchsia', alpha=0.30 )

ax_2[1].fill_between( horizon_steps, [mc1_500[3]-1.96*mc1_500_std[3], mc2_500[3]-1.96*mc2_500_std[3], mc3_500[3]-1.96*mc3_500_std[3], mc4_500[3]-1.96*mc4_500_std[3], mc5_500[3]-1.96*mc5_500_std[3], mc6_500[3]-1.96*mc6_500_std[3], mc7_500[3]-1.96*mc7_500_std[3], mc8_500[3]-1.96*mc8_500_std[3], mc9_500[3]-1.96*mc9_500_std[3], mc10_500[3]-1.96*mc10_500_std[3] ], [mc1_500[3]+1.96*mc1_500_std[3], mc2_500[3]+1.96*mc2_500_std[3], mc3_500[3]+1.96*mc3_500_std[3], mc4_500[3]+1.96*mc4_500_std[3], mc5_500[3]+1.96*mc5_500_std[3], mc6_500[3]+1.96*mc6_500_std[3], mc7_500[3]+1.96*mc7_500_std[3], mc8_500[3]+1.96*mc8_500_std[3], mc9_500[3]+1.96*mc9_500_std[3], mc10_500[3]+1.96*mc10_500_std[3] ], color='lime', alpha=0.30 )
ax_2[1].fill_between( horizon_steps, [mc1_5000[3]-1.96*mc1_5000_std[3], mc2_5000[3]-1.96*mc2_5000_std[3], mc3_5000[3]-1.96*mc3_5000_std[3], mc4_5000[3]-1.96*mc4_5000_std[3], mc5_5000[3]-1.96*mc5_5000_std[3], mc6_5000[3]-1.96*mc6_5000_std[3], mc7_5000[3]-1.96*mc7_5000_std[3], mc8_5000[3]-1.96*mc8_5000_std[3], mc9_5000[3]-1.96*mc9_5000_std[3], mc10_5000[3]-1.96*mc10_5000_std[3] ], [mc1_5000[3]+1.96*mc1_5000_std[3], mc2_5000[3]+1.96*mc2_5000_std[3], mc3_5000[3]+1.96*mc3_5000_std[3], mc4_5000[3]+1.96*mc4_5000_std[3], mc5_5000[3]+1.96*mc5_5000_std[3], mc6_5000[3]+1.96*mc6_5000_std[3], mc7_5000[3]+1.96*mc7_5000_std[3], mc8_5000[3]+1.96*mc8_5000_std[3], mc9_5000[3]+1.96*mc9_5000_std[3], mc10_5000[3]+1.96*mc10_5000_std[3] ], color='cyan', alpha=0.30 )
ax_2[1].fill_between( horizon_steps, [mc1_50000[3]-1.96*mc1_50000_std[3], mc2_50000[3]-1.96*mc2_50000_std[3], mc3_50000[3]-1.96*mc3_50000_std[3], mc4_50000[3]-1.96*mc4_50000_std[3], mc5_50000[3]-1.96*mc5_50000_std[3], mc6_50000[3]-1.96*mc6_50000_std[3], mc7_50000[3]-1.96*mc7_50000_std[3], mc8_50000[3]-1.96*mc8_50000_std[3], mc9_50000[3]-1.96*mc9_50000_std[3], mc10_50000[3]-1.96*mc10_50000_std[3] ], [mc1_50000[3]+1.96*mc1_50000_std[3], mc2_50000[3]+1.96*mc2_50000_std[3], mc3_50000[3]+1.96*mc3_50000_std[3], mc4_50000[3]+1.96*mc4_50000_std[3], mc5_50000[3]+1.96*mc5_50000_std[3], mc6_50000[3]+1.96*mc6_50000_std[3], mc7_50000[3]+1.96*mc7_50000_std[3], mc8_50000[3]+1.96*mc8_50000_std[3], mc9_50000[3]+1.96*mc9_50000_std[3], mc10_50000[3]+1.96*mc10_50000_std[3] ], color='fuchsia', alpha=0.30 )
# plt.show()
...
ax_4[0].plot(horizon_steps, [mc1_500[6], mc2_500[6], mc3_500[6], mc4_500[6], mc5_500[6], mc6_500[6], mc7_500[6], mc8_500[6], mc9_500[6], mc10_500[6] ], 'green'  )
ax_4[0].plot(horizon_steps, [mc1_5000[6], mc2_5000[6], mc3_5000[6], mc4_5000[6], mc5_5000[6], mc6_5000[6], mc7_5000[6], mc8_5000[6], mc9_5000[6], mc10_5000[6] ], 'b'  )
ax_4[0].plot(horizon_steps, [mc1_50000[6], mc2_50000[6], mc3_50000[6], mc4_50000[6], mc5_50000[6], mc6_50000[6], mc7_50000[6], mc8_50000[6], mc9_50000[6], mc10_50000[6] ], 'fuchsia'  )
ax_4[0].plot(horizon_steps, [foresee1[6], foresee2[6], foresee3[6], foresee4[6], foresee5[6], foresee6[6], foresee7[6], foresee8[6], foresee9[6], foresee10[6] ], 'y'  )
ax_4[0].plot(horizon_steps, [foresee_genut1[6], foresee_genut2[6], foresee_genut3[6], foresee_genut4[6], foresee_genut5[6], foresee_genut6[6], foresee_genut7[6], foresee_genut8[6], foresee_genut9[6], foresee_genut10[6] ], 'salmon'  )

ax_4[1].plot(horizon_steps, [mc1_500[7], mc2_500[7], mc3_500[7], mc4_500[7], mc5_500[7], mc6_500[7], mc7_500[7], mc8_500[7], mc9_500[7], mc10_500[7] ], 'green'  )
ax_4[1].plot(horizon_steps, [mc1_5000[7], mc2_5000[7], mc3_5000[7], mc4_5000[7], mc5_5000[7], mc6_5000[7], mc7_5000[7], mc8_5000[7], mc9_5000[7], mc10_5000[7] ], 'b'  )
ax_4[1].plot(horizon_steps, [mc1_50000[7], mc2_50000[7], mc3_50000[7], mc4_50000[7], mc5_50000[7], mc6_50000[7], mc7_50000[7], mc8_50000[7], mc9_50000[7], mc10_50000[7] ], 'fuchsia'  )
ax_4[1].plot(horizon_steps, [foresee1[7], foresee2[7], foresee3[7], foresee4[7], foresee5[7], foresee6[7], foresee7[7], foresee8[7], foresee9[7], foresee10[7] ], 'y'  )
ax_4[1].plot(horizon_steps, [foresee_genut1[7], foresee_genut2[7], foresee_genut3[7], foresee_genut4[7], foresee_genut5[7], foresee_genut6[7], foresee_genut7[7], foresee_genut8[7], foresee_genut9[7], foresee_genut10[7] ], 'salmon'  )


ax_4[0].fill_between( horizon_steps, [mc1_500[6]-1.96*mc1_500_std[6], mc2_500[6]-1.96*mc2_500_std[6], mc3_500[6]-1.96*mc3_500_std[6], mc4_500[6]-1.96*mc4_500_std[6], mc5_500[6]-1.96*mc5_500_std[6], mc6_500[6]-1.96*mc6_500_std[6], mc7_500[6]-1.96*mc7_500_std[6], mc8_500[6]-1.96*mc8_500_std[6], mc9_500[6]-1.96*mc9_500_std[6], mc10_500[6]-1.96*mc10_500_std[6] ], [mc1_500[6]+1.96*mc1_500_std[6], mc2_500[6]+1.96*mc2_500_std[6], mc3_500[6]+1.96*mc3_500_std[6], mc4_500[6]+1.96*mc4_500_std[6], mc5_500[6]+1.96*mc5_500_std[6], mc6_500[6]+1.96*mc6_500_std[6], mc7_500[6]+1.96*mc7_500_std[6], mc8_500[6]+1.96*mc8_500_std[6], mc9_500[6]+1.96*mc9_500_std[6], mc10_500[6]+1.96*mc10_500_std[6] ], color='lime', alpha=0.30 )
ax_4[0].fill_between( horizon_steps, [mc1_5000[6]-1.96*mc1_5000_std[6], mc2_5000[6]-1.96*mc2_5000_std[6], mc3_5000[6]-1.96*mc3_5000_std[6], mc4_5000[6]-1.96*mc4_5000_std[6], mc5_5000[6]-1.96*mc5_5000_std[6], mc6_5000[6]-1.96*mc6_5000_std[6], mc7_5000[6]-1.96*mc7_5000_std[6], mc8_5000[6]-1.96*mc8_5000_std[6], mc9_5000[6]-1.96*mc9_5000_std[6], mc10_5000[6]-1.96*mc10_5000_std[6] ], [mc1_5000[6]+1.96*mc1_5000_std[6], mc2_5000[6]+1.96*mc2_5000_std[6], mc3_5000[6]+1.96*mc3_5000_std[6], mc4_5000[6]+1.96*mc4_5000_std[6], mc5_5000[6]+1.96*mc5_5000_std[6], mc6_5000[6]+1.96*mc6_5000_std[6], mc7_5000[6]+1.96*mc7_5000_std[6], mc8_5000[6]+1.96*mc8_5000_std[6], mc9_5000[6]+1.96*mc9_5000_std[6], mc10_5000[6]+1.96*mc10_5000_std[6] ], color='cyan', alpha=0.30 )
ax_4[0].fill_between( horizon_steps, [mc1_50000[6]-1.96*mc1_50000_std[6], mc2_50000[6]-1.96*mc2_50000_std[6], mc3_50000[6]-1.96*mc3_50000_std[6], mc4_50000[6]-1.96*mc4_50000_std[6], mc5_50000[6]-1.96*mc5_50000_std[6], mc6_50000[6]-1.96*mc6_50000_std[6], mc7_50000[6]-1.96*mc7_50000_std[6], mc8_50000[6]-1.96*mc8_50000_std[6], mc9_50000[6]-1.96*mc9_50000_std[6], mc10_50000[6]-1.96*mc10_50000_std[6] ], [mc1_50000[6]+1.96*mc1_50000_std[6], mc2_50000[6]+1.96*mc2_50000_std[6], mc3_50000[6]+1.96*mc3_50000_std[6], mc4_50000[6]+1.96*mc4_50000_std[6], mc5_50000[6]+1.96*mc5_50000_std[6], mc6_50000[6]+1.96*mc6_50000_std[6], mc7_50000[6]+1.96*mc7_50000_std[6], mc8_50000[6]+1.96*mc8_50000_std[6], mc9_50000[6]+1.96*mc9_50000_std[6], mc10_50000[6]+1.96*mc10_50000_std[6] ], color='fuchsia', alpha=0.30 )

ax_4[1].fill_between( horizon_steps, [mc1_500[7]-1.96*mc1_500_std[7], mc2_500[7]-1.96*mc2_500_std[7], mc3_500[7]-1.96*mc3_500_std[7], mc4_500[7]-1.96*mc4_500_std[7], mc5_500[7]-1.96*mc5_500_std[7], mc6_500[7]-1.96*mc6_500_std[7], mc7_500[7]-1.96*mc7_500_std[7], mc8_500[7]-1.96*mc8_500_std[7], mc9_500[7]-1.96*mc9_500_std[7], mc10_500[7]-1.96*mc10_500_std[7] ], [mc1_500[7]+1.96*mc1_500_std[7], mc2_500[7]+1.96*mc2_500_std[7], mc3_500[7]+1.96*mc3_500_std[7], mc4_500[7]+1.96*mc4_500_std[7], mc5_500[7]+1.96*mc5_500_std[7], mc6_500[7]+1.96*mc6_500_std[7], mc7_500[7]+1.96*mc7_500_std[7], mc8_500[7]+1.96*mc8_500_std[7], mc9_500[7]+1.96*mc9_500_std[7], mc10_500[7]+1.96*mc10_500_std[7] ], color='lime', alpha=0.30 )
ax_4[1].fill_between( horizon_steps, [mc1_5000[7]-1.96*mc1_5000_std[7], mc2_5000[7]-1.96*mc2_5000_std[7], mc3_5000[7]-1.96*mc3_5000_std[7], mc4_5000[7]-1.96*mc4_5000_std[7], mc5_5000[7]-1.96*mc5_5000_std[7], mc6_5000[7]-1.96*mc6_5000_std[7], mc7_5000[7]-1.96*mc7_5000_std[7], mc8_5000[7]-1.96*mc8_5000_std[7], mc9_5000[7]-1.96*mc9_5000_std[7], mc10_5000[7]-1.96*mc10_5000_std[7] ], [mc1_5000[7]+1.96*mc1_5000_std[7], mc2_5000[7]+1.96*mc2_5000_std[7], mc3_5000[7]+1.96*mc3_5000_std[7], mc4_5000[7]+1.96*mc4_5000_std[7], mc5_5000[7]+1.96*mc5_5000_std[7], mc6_5000[7]+1.96*mc6_5000_std[7], mc7_5000[7]+1.96*mc7_5000_std[7], mc8_5000[7]+1.96*mc8_5000_std[7], mc9_5000[7]+1.96*mc9_5000_std[7], mc10_5000[7]+1.96*mc10_5000_std[7] ], color='cyan', alpha=0.30 )
ax_4[1].fill_between( horizon_steps, [mc1_50000[7]-1.96*mc1_50000_std[7], mc2_50000[7]-1.96*mc2_50000_std[7], mc3_50000[7]-1.96*mc3_50000_std[7], mc4_50000[7]-1.96*mc4_50000_std[7], mc5_50000[7]-1.96*mc5_50000_std[7], mc6_50000[7]-1.96*mc6_50000_std[7], mc7_50000[7]-1.96*mc7_50000_std[7], mc8_50000[7]-1.96*mc8_50000_std[7], mc9_50000[7]-1.96*mc9_50000_std[7], mc10_50000[7]-1.96*mc10_50000_std[7] ], [mc1_50000[7]+1.96*mc1_50000_std[7], mc2_50000[7]+1.96*mc2_50000_std[7], mc3_50000[7]+1.96*mc3_50000_std[7], mc4_50000[7]+1.96*mc4_50000_std[7], mc5_50000[7]+1.96*mc5_50000_std[7], mc6_50000[7]+1.96*mc6_50000_std[7], mc7_50000[7]+1.96*mc7_50000_std[7], mc8_50000[7]+1.96*mc8_50000_std[7], mc9_50000[7]+1.96*mc9_50000_std[7], mc10_50000[7]+1.96*mc10_50000_std[7] ], color='fuchsia', alpha=0.30 )
# plt.show()
