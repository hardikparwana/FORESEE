...
from lf_utils.ut_utilsJIT import *
from lf_utils.gp_utils import *
...
def compute_A1_b1_tensor(robotsJ, robotsK, robotsJ_state, robotsK_state, leader_pose_dot):
    
    A1, b1 = unicycle_SI2D_clf_cbf_fov_evaluator(robotsJ_state, robotsK_state, leader_pose_dot, robotsJ.k_torch, robotsJ.alpha_torch)
...
def get_future_reward( follower, leader, gp, t = 0, noise = torch.tensor(0), enforce_input_constraints = False ):
    ...
    for i in range(H):       
        ...
        # t0 = time.time()
        leader_xdot_states, leader_xdot_weights = traced_sigma_point_expand_JIT( follower_states[i], leader_states[i], leader_weights[i], torch.tensor(tp), gp )
...
def simulate_scenario(movie_name = 'test.mp4', adapt = False, noise = 0.1, enforce_input_constraints = False):
    ...
    train_x = np.array( [ 0 ] ).reshape(1,-1)
    train_y = np.array( [ 0, 0 ] ).reshape(1, -1)
    gp = MultitaskGPModel(torch.tensor(train_x, dtype=torch.float), torch.tensor(train_y, dtype=torch.float), likelihood, num_tasks=2)
    gp.eval()
    likelihood.eval()
    leader_pose = np.copy(leader.X)
    ...
    with writer.saving(fig, movie_name, 100): 
        ...
        for i in range(num_steps):
            ...
            if i % outer_loop != 0 or i<learn_period:
                ...
                if not estimator_init:
                    leader_pose_dot = torch.tensor( [0,0], dtype=torch.float ).reshape(-1,1)
                else:
                    cur_t = torch.tensor( t, dtype = torch.float )
                    # print(f"from controller input_x:{cur_t}, gp x:{self.gp.train_inputs}")
                    prediction = gp( cur_t.reshape(1,-1) )
                    leader_pose_dot = prediction.mean.reshape(-1,1)
                A, B = compute_A1_b1_tensor( follower, leader, follower.X_torch, leader.X_torch, leader_pose_dot )
                # print("hello2")
                solution, deltas = cbf_controller_layer( u_ref, A, B )
                ...
                leader_pose_previous = np.copy( leader.X_prev )
                leader_pose = np.copy(leader.X)

                diff = leader_pose - leader_pose_previous
                new_y = diff / dt_inner
                ...
                if i % gp_learn_loop == 0 and i > 0:
                    ...
                    idxs = np.random.choice(np.shape( train_x )[0], min( np.shape(train_x)[0], 30 ), replace=False)
            ...
            # Low Frequency tuning
                ...
                if adapt:
                    ...
                    success = False
                    while not success:
                        maintain_constraints, improve_constraints, success, reward = get_future_reward( follower, leader, gp, t = t, noise = torch.tensor(noise), enforce_input_constraints = enforce_input_constraints)                    
