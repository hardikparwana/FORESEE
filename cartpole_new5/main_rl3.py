...
from cartpole_new5.gp_utils import * # initialize_gp, train_gp, predict_gp
from cartpole_new5.cartpole_policy import policy, policy9, policy_grad, random_exploration, Sum_of_gaussians_initialize, random_policy
from cartpole_new5.ut_utils.ut_utils import *

# visualization
from robot_models.custom_cartpole_constrained import CustomCartPoleEnv
from robot_models.cartpole2D_openai import step, get_next_states_from_dynamics, step_with_diffrax
from cartpole_new5.gym_wrappers.record_video import RecordVideo
...
#     #train using scipy ###########################
    ...
    if use_adam:
        ...
        costs_adam = []
        moving_average_cost_improvement = 0
        for j in range(n_restarts):
            ...
            for i in range(iters_adam[run] + 1):
                ...
                params_policy = np.copy(params_policy_temp)
                # opt_state = gradient_transform.init(params_policy)
